...
@file: CCC initiator demo script (Normal CCC protocol, CCC-TS-101 Test Vector)
...
def main():
    ...
    ccc_demo_app.uci_core_get_config([])
    result = ccc_demo_app.wait_response(timeout_ms=400)
    if result.status == EnumUciStatus.UCI_STATUS_OK:
        log_i("License verification successful!")
    else:
    ...
    ccc_car_keys_param = CCCSessionParam(session_id=0x12345678, device_type=EnumCCCDeviceType.CCC_DEVICE_TYPE_CONTROLLER.value, uwb_config_id=0,
                                         device_role=EnumCCCDeviceRole.CCC_DEVICE_ROLE_INITIATOR, anchor_num=1, sts_index0=0x075BCD15, slots_per_rr=6, ranging_interval=960, ccc_config_quirks=1,
    ...
    ccc_demo_app.uci_session_get_app_config(0x12345678, [])
    result = ccc_demo_app.wait_response(timeout_ms=400)
    if result.status == EnumUciStatus.UCI_STATUS_OK:
        log_i("License verification successful!")
    else:
